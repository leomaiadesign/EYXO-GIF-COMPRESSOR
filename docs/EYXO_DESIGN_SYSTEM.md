...
### Tags / Status Badges
- **Texto:** Sempre CAIXA ALTA (`text-transform: uppercase`).
- **Background:** `--eyxo-border` para neutro/pendente. `--eyxo-blue` para processando. Verde (`#00FF00`) para pronto/concluído.
- **Tipografia:** Negrito (`font-weight: 600`), texto pequeno (`font-size: 11px`), letras espaçadas (`letter-spacing: 0.5px`).

### Acessibilidade de Contraste
- **Regra de Ouro:** NUNCA aplicar um texto na cor Azul (`--eyxo-blue`) diretamente sobre fundos escuros (`--eyxo-bg` ou `--eyxo-surface`), pois isso gera baixíssima legibilidade e quebra o contraste.
- Use a cor azul APENAS em fundos onde há contraste real (como dentro de botões com fundo azul e texto branco, ou em tags com fundo azul claro e texto escuro, se houver).
- Textos em destaque sobre fundo escuro devem sempre usar Branco (`--eyxo-text`). Bordas podem ser azuis para indicar foco/estado ativo.
